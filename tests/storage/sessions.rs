...
#[cfg(windows)]
#[tokio::test]
async fn generated_session_database_and_sidecar_reparse_points_are_preserved_and_rejected() {
    use std::{
        fs,
        os::windows::fs::{MetadataExt, symlink_file},
    };
    ...
    for name in [
        ...
        "session.sqlite3-wal",
        "session.sqlite3-shm",
        "session.sqlite3-journal",
    ] {
        let fixture = Fixture::new();
        let store = SessionStore::open(fixture.root.clone()).await.unwrap();
        let created = store
            .create_session(request(OperationId::new(), "synthetic-session-canary"))
            .await
            .unwrap();
        let path = session_path(&fixture, created.session_id());
        ...
        let link = path.with_file_name(name);
        if name == "session.sqlite3" {
            // Keep a valid database behind the link so following it cannot mask rejection.
            fs::rename(&path, &target).unwrap();
        } else {
            fixtures::file(&target, b"synthetic-session-sidecar-canary");
        }
        let target_bytes = fs::read(&target).unwrap();
        symlink_file(&target, &link).expect("Windows file symlink fixture must be created");
        ...
        store.close().await.unwrap();
        let preserved = fs::symlink_metadata(&link).unwrap();
        assert_ne!(
        ...
        for suffix in ["-wal", "-shm", "-journal"] {
            let sidecar = path.with_file_name(format!("session.sqlite3{suffix}"));
            if sidecar != link {
                assert_eq!(
