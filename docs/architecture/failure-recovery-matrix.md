...
`unavailable` and `missing` are authoritative storage-boundary states. Normal session open, every client read/recovery, direct command acceptance, and direct append use per-session coordination shared with catalog status transitions. Existing homes are realpath-canonicalized before catalog/session paths and coordinator lookup, so static symlink aliases of one physical home converge on the same boundary. The guard spans the final catalog check and the complete session-worker request: a transition waits for earlier work, later work waits for the transition and rechecks its result, and rejection closes any retained lazy database handle. A stale server/router precheck or a queued worker request therefore cannot authorize work after a non-ready transition commits. Generic reconciliation requires an existing ready row: it cannot insert an absent row or promote a non-ready row, even when given a matching stale inspection. Only module-internal reconciliation capabilities may reconstruct an absent row: incomplete-creation recovery requires the durable `creating` reservation, while validated repair used by the complete bounded scanner requires all discovery and provenance checks to pass. That operation is held in module closure rather than a runtime-visible client field or symbol. The manager's worker pool and each client's pool/hooks use JavaScript private fields, and the normal storage package entry point exports no worker-pool or client constructor. Internal test access is explicitly `NODE_ENV=test` gated and is not a package export. Catalog and session databases remain separately transactional; no cross-database transaction is introduced.
...
HTTP sockets, rejected upgrades, WebSockets, actor cancellation, scheduler drain, catalog observation drain, and worker close all have finite deadlines. A timed-out component is isolated or force-closed where a hard boundary exists, its diagnostic is recorded, and later shutdown phases continue. A fully force-closed HTTP/WebSocket transport timeout is successful fault-domain isolation and may still complete shutdown with exit zero; an unresolved internal phase or failed transport isolation is aggregated into `server_shutdown_diagnostic` and exits nonzero. In-process provider/tool work that ignores cancellation has no safe smaller isolation boundary in v0.1, so Wi exits nonzero without falsely terminalizing the run; restart recovery conservatively handles its durable nonterminal state. The process suite sends real SIGTERM during both provider and pure-tool execution on Linux. For the pure tool, fail-closed process exit leaves one started ledger row; restart reconciles and completes that same row at attempt 2 without an `outcome_unknown` event. Process fixtures use detached Linux process groups, bounded waits, escalation, verified group disappearance, and only then temporary-home deletion. Harness stdout and stderr diagnostics are fixed 64-KiB byte tails with exact total-byte counts, truncation flags, and streaming SHA-256 hashes; readiness matching uses a streaming marker capped at 4 KiB and never scans retained output. Real-server object IPC uses a strict test-control protocol: one retained message is capped at a conservative 64-KiB estimate, depth 32, 4,096 nodes, 16,384 code units per string, and 128 code units for its type. The estimate charges complete JSON string/key escaping and complete finite-number encodings plus structural overhead, so the UTF-8 JSON encoding of retained messages cannot exceed the charged aggregate. `NaN`, positive infinity, and negative infinity are protocol-invalid: retention replaces them with a bounded rejection summary, and outbound preflight rejects them before `child.send()` rather than allowing JSON-mode IPC to coerce them to `null`. Pending retention is capped at 128 messages and 256 KiB estimated aggregate; history is capped at 256 messages and 512 KiB estimated aggregate. A rejected value is released immediately after the callback and only a fixed 512-code-unit preview, type when bounded, reason, observed counters, and streaming SHA-256 are retained; awaited small controls are preferred over unawaited noise during pending eviction. Timeout text includes only these bounded diagnostics. Outbound parent controls are copied once into a bounded plain-data snapshot; accessors and unsupported values are rejected, proxy descriptor results are captured into the snapshot, arrays capture one own length descriptor and use null-prototype indexed snapshots, and `child.send()` receives that exact validated snapshot rather than the dynamic source object. This is a per-control representation bound for trusted test callers; it does not bound aggregate in-flight buffering across repeated parent `child.send()` calls. Node object IPC necessarily deserializes before the JavaScript callback, so this is a retained-memory and strict trusted-fixture protocol boundary, not a pre-deserialization defense against a hostile child; replacing it with a framed pipe is required if fixture children become untrusted. Crossing a retention cap does not itself kill the child; timeout and verified tree cleanup remain authoritative. Generic fixture runners retain and verify that boundary after every leader close, including a normal zero exit, before removing the fixture from cleanup tracking. On Linux, a test-support watchdog installs an inherited owner-pipe EOF watcher before a preload gate releases fixture code. The watchdog owns the registered detached process-group identity across direct-leader exit, reclaims that group if the harness owner dies, and is released only after normal or explicit cleanup; setup failure never releases fixture code unmanaged. Release has one shared in-flight attempt rather than a permanently cached result. Failure removes listeners and clears only that attempt while retaining ownership; a later cleanup sends a new request, or safely escalates a disconnected watchdog, and removes ownership only after both group and watchdog disappearance are verified. A nested owner-death process probe verifies that Linux owner-pipe EOF reclaims both the fixture leader and descendant before ordinary outer-harness cleanup begins. There is no leader-only fallback that can claim unverified cleanup. Noncooperative replay disconnects are isolated cleanly without waiting for the retained hook; a noncooperative catalog observation exhausts the shared deadline, exits nonzero with a shutdown diagnostic, and remains recoverable on restart.
