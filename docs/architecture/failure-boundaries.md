...
Because catalog discovery is foundational, normal startup may stop. Before an existing catalog is opened read-write, SQLite recovery, integrity checking, and migration validation run on a disposable copy of the database and WAL/SHM sidecars. Startup then uses a two-phase handshake: SQLite acquires the canonical handle without running configuration, the worker synchronously compares the current directory and DB/WAL/SHM identities with the validated snapshot, and only a match may proceed to recovery-capable PRAGMAs or queries. A mismatch closes the inert raw handle. Missing bootstrap uses no-overwrite/no-follow reservation before the same prepared-open flow. Fixed catalog-startup classifications replace raw filesystem messages before RPC, preventing canonical-home and probe-path disclosure. Failed validation leaves the canonical files unopened and unchanged. Automatic quarantine/replacement is forbidden because Node exposes no cross-platform handle-relative, no-follow, no-overwrite move. With Wi stopped, an operator may restore the catalog or deliberately relocate it outside Wi before using the missing-catalog rebuild path.
...
A sudden crash may bypass these steps; recovery tests cover durable consequences. The complete process project is a required CI job on both Ubuntu and Windows, including native Windows Job Object escalation, owner-death cleanup, and directory-junction scanner containment. Process fixtures share bounded process-tree cleanup: detached POSIX groups or named Windows Job Objects configured with `JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE`, escalation, verified owned-boundary disappearance, and temporary-home deletion only after cleanup settles. POSIX uses operating-system signals plus a test-support-only owner watchdog. An inherited owner pipe reaches the watchdog before a gated preload releases fixture code; owner EOF reclaims the registered fixture process group even after its direct leader exits. Normal completion releases the watchdog after verified group cleanup, and setup failure leaves fixture code gated and fails closed. Windows graceful requests travel over a per-child authenticated inherited IPC channel and invoke the fixture's SIGTERM/SIGINT handler; Job Object termination is used only for force kill or failed graceful cleanup. The Windows test-only preload joins the job before fixture code executes, while the parent retains and queries the job after direct-leader exit; cleanup without Job Object ownership fails closed rather than treating leader exit as descendant proof. Generic fixture runners likewise verify and release their owned POSIX group or Windows job after normal leader exit instead of dropping ownership from cleanup tracking. Both fixture harnesses retain only fixed 64-KiB stdout/stderr diagnostic tails plus total-byte, truncation, and streaming-hash metadata. Real-server pending and historical IPC queues have independent fixed message caps, and readiness matching is streaming with a fixed marker bound rather than a search over accumulated output. Output volume alone never triggers child termination; existing deadlines and verified process-tree cleanup remain authoritative. A graceful-shutdown-during-tool probe proves that a noncooperative pure tool exits fail-closed and that restart safely retries the same durable ledger row rather than inventing a terminal outcome. Separate retained-gate probes show that replay is isolated cleanly while a noncooperative catalog observation reaches the shared deadline, reports failure, and recovers after restart. When restart recovery terminalizes related provider-step, tool, and run state as one causal boundary, those events share one diagnostic ID and one redacted `session_recovery_interrupted` server record. When a provider-step terminal event committed before the crash but its run terminal event did not, the provider-step projection retains the canonical diagnostic ID and run adoption reuses it. The complete failpoint inventory, scanner trigger policy, catalog/project limitations, and shutdown order are documented in [the Milestone 7 failure and recovery matrix](failure-recovery-matrix.md).
