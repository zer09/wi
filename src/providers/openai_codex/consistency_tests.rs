//! Former CLI regressions retained at the adapter's normalized-event boundary.
use super::consistency as collect_lifecycle;
use crate::{
    DeltaKind, EventEnvelope, InputItem, ItemKind, ModelResponse, OutputItem, ProviderEvent,
    ProviderSession, RequestReceipt, ResponseOutcome, Result, SessionControl,
...
async fn collect_accepts_recovered_effective_output_without_native_guard_false_failure() {
    for mode in [false, true] {
        ...
        if let ProviderEvent::ResponseFinished { response } = &mut terminal {
            ...
            response.output_provenance = crate::OutputProvenance::ValidatedOutputItemDone;
        }
